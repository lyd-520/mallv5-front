...
import { Message,Loading,Autocomplete} from 'element-ui'
...
axios.interceptors.request.use(config => {
  // if (config.baseURL!="baiduMapApi")  return config;
...
axios.interceptors.response.use(function(response){ 
  ...
  if(res.code == 200||res.code==666){
    return res.data;
  }
  else if (res.message=="ok"||res.status==0){
      return res;
  }
