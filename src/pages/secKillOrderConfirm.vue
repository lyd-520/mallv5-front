...
export default{
  ...
  mounted(){
    ...
    this.flashPromotionId=localStorage.getItem("flashPromotionId"),
    this.getAddressList();
    this.getCartList();
    // this.getCouponList();
  },
  ...
  methods:{
    querySearch(queryString, cb) {
      ...
      let url =
          ...
          "&region=" +
          region +
          "&output=json&ak=qgcCiqZoU1yM6CxYRFPUKN0pUz4Pji1W";

      this.axios
          .get(url, { baseURL: "baiduMapApi",
          })
          // 设置代理前缀
          .then( (res) => {
            if (res!=null){
    ...
    getCartList(){
        ...
        // var confirmOrderurl = '/order/'+this.token+'/generateConfirmOrder';
        var confirmOrderurl = '/skCart/generateConfirmOrder';
       this.axios.post(confirmOrderurl,
           Qs.stringify({productId: this.id, flashPromotionId: this.flashPromotionId},
                  {indices: false}),
    ...
    // 订单提交
    orderSubmit(){
      let address = this.list[this.checkIndex];
    ...
    this.axios.post('/skOrder/generateOrder',{
// this.axios.post('/order/'+this.token+'/generateOrder',{
                orderId: this.orderId ,
                orderItemId: this.orderItemId   ,
                flashPromotionId:this.flashPromotionId,
                productId:this.id,
                memberReceiveAddress: address,
                payType: 1,
              
              },{headers: {'Content-Type': 'application/json','memberId':this.memberId
      }}).then((res)=>{
               //同步               
              ...
              //异步 - 状态同步页面               
                 ...
                 this.$router.push({path:"/order/status/"+this.orderId+'/'+this.id});
               }else{
          ...
          .addr-list{
            ...
            .addr-add{
              ...
              .icon-add{
                ...
                height:30px;
                border-radius:50%;
                background:url('/resource/imgs/icon-add.png') #E0E0E0 no-repeat center;
