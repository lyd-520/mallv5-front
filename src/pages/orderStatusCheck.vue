...
<script>
  ...
  export default{
    ...
    data(){
      return {
         otimer:function(){},
         productId: this.$route.params.itemIds,
    ...
    methods:{
      getOrderStatus(){
         ...
         this.axios.post('/skOrder/checkOrder', Qs.stringify({
           orderId:this.orderId,
           productId:this.productId
        },{indices: false}),{headers: {'memberId':this.memberId}}).then((res)=>{
            if(res==null){
                this.resultText="查询失败，请返回";
                this.destroyed();
              return ;
            }
            if(res===-1){
